...
// Get the case_no for redirect
$stmt = $conn->prepare("SELECT case_no FROM cases WHERE id = ?");
$stmt->bind_param("i", $case_id);
$stmt->execute();
$case_no_result = $stmt->get_result();
$case_no_row = $case_no_result->fetch_assoc();
$case_no = $case_no_row ? $case_no_row['case_no'] : '';

// Redirect back to case detail page
header("Location: ../../client/case_detail.php?case_no=" . urlencode($case_no));
exit();
?>